...
const countryCodes = [
	...
	{ name: "Cyprus", dialCode: "+537", code: "CY", flag: "🇨🇾" },
...
const available = availableCountries.map((x) => x.toLowerCase());

console.log(`Countries: ${countryCodes.length}`);
console.log(`Available: ${availableCountries.length}`);

const filteredCountries = countryCodes.filter((x) =>
...
filteredCountries.forEach((X) => {
	const occurences = filteredCountries
		.filter((x) => X.dialCode === x.dialCode)
		.map((occurence) => occurence.name);

	if (
		occurences.length > 1 &&
		!appearsMoreThanOnce.map((occ) => occ.dialCode).includes(X.dialCode)
	) {
